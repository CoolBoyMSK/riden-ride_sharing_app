...
export const updatePassengerProfile = async (user, body, file, resp) => {
  ...
  try {
    ...
    const myUser = await findUserById(user._id);
    ...
    if (
      ...
      messages.nameMessage = 'Passenger name updated successfully';
    }
    ...
    const ALLOWED_FIELDS = ['gender', 'name', 'profileImg'];
...
export const updateDriverProfile = async (user, body, file, resp) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  try {
    const update = { ...body };
    ...
    if (
      ...
      const notify = await createAdminNotification({
        ...
        message: `A driver ${user.name} has submitted update request for username`,
      ...
      if (!notify) {
        console.log('Failed to send profile update notification');
      }
    ...
    if (file) {
      ...
      if (!newImageSource) {
        ...
        if (!same) {
          ...
          const notify = await createAdminNotification({
            title: 'New Profile Update Request',
            message: `A driver ${user.name} has submitted update request for profile image`,
            metadata: { requestResult },
            module: 'driver_management',
            type: 'ALERT',
            actionLink: null,
